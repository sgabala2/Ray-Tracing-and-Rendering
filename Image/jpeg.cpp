...
namespace Image
{
	void JPEGReadImage( std::string fileName , Image32& img )
	{
		...
		if( !fp ) THROW( "Failed to open file for reading: " , fileName );
	...
	void JPEGWriteImage( const Image32& img , std::string fileName , int quality )
	{
		...
		if( !fp ) THROW( "Failed to open file for writing: " , fileName );
	...
	void JPEGReadImage( FILE *fp , Image32& img )
	{
		...
		if( cinfo.output_components!=1 && cinfo.output_components!=3 ) THROW( "Wrong number of components: " , cinfo.output_components );
	...
	void JPEGWriteImage( const Image32& img , FILE *fp , int quality )
	{
		...
		*/
		...
		if( !pixels ) THROW( "Failed to allocate pixels[" , row_stride , "]" );
